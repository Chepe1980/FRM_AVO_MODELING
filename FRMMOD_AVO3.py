...
if uploaded_file:
    try:
        ...
        with tab2:
            ...
            titles = ['Brine Case', 'Oil Case', 'Gas Case']
            
            for i, (fluid, color, title) in enumerate(zip(fluids, colors, titles)):
                ax[i].scatter(logs[f'IP_FRM{fluid}'], logs[f'VPVS_FRM{fluid}'], 
                            c=logs[f'LFC_{fluid}'], cmap=cmap, vmin=0, vmax=4, alpha=0.6)
        ...
        with tab4:
            ...
            st.warning("Oil & Gas case implementation would follow the same pattern as the Brine case")
            st.info("This section would show similar AVO analysis for oil and gas scenarios")
